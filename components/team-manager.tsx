  ...
  const addMember = (teamId: string) => {
    if (memberInput.trim()) {
      onTeamsChange(
        teams.map((team) =>
          team.id === teamId
            ? { ...team, members: [...team.members, memberInput.trim()] }
            : team
        )
      );
      setMemberInput("");
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {teams.map((team) => (
          <Card
            key={team.id}
            className="bg-jeopardy-blue border-jeopardy-gold/30 p-4"
          >
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-lg font-bold text-jeopardy-gold">
                {team.name}
              </h4>
              <div className="flex gap-2">
                <Button
                  ...
                  className="bg-jeopardy-blue border-jeopardy-gold/30 text-white hover:bg-jeopardy-gold/20 hover:border-jeopardy-gold"
                >
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {team.members.map((member, idx) => (
                  ...
                  >
                    ...
                    {editingTeam === team.id && (
                      <button
                        onClick={() => removeMember(team.id, idx)}
